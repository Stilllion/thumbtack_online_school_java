...
public class TestGetEmployeesAnySkill
{
    Server s = new Server();
    Gson gson = new Gson();
    ...
    @Test
    public void testGetEmployeesAllSkills()
    {
        List<Employee> registeredEmployee;
        List<Employee> applicableEmployees = new ArrayList<>();

        // Add vacancy of Java programmer with following skills:
        // Java - lvl 5, English - lvl 4 (both mandatory), HTML&CSS - lvl 2 not mandatory
        UUID employerToken = addVacancy();

        // Register 5 employees with different skills, only the first and the third would
        ...
        GetEmployeesDtoResponse response = gson.fromJson(s.getEmployeesAnySkill(gson.toJson(
